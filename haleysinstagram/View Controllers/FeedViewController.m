    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray * _Nullable posts, NSError * _Nullable error) {
        ...
        if (error != nil) {
            UIAlertController *alert = [ErrorAlert
                                        getErrorAlertWithTitle:@"Error loading feed"
...
- (void)setupPullToRefresh {
    ...
    [self.tableView insertSubview:self.refreshController
                          atIndex:0];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    UITapGestureRecognizer *labelGesture = [[UITapGestureRecognizer alloc]
                                            initWithTarget:self
                                            action:@selector(didTapUsernameLabel:)];
...
- (IBAction)didTapLogout:(id)sender {
    ...
    [MyUser logOutInBackgroundWithBlock:^(NSError * _Nullable error) {
        if (error != nil) {
            UIAlertController *alert = [ErrorAlert
                                        getErrorAlertWithTitle:@"Error logging out"
                                        withMessage:error.localizedDescription];
            [self presentViewController:alert
                               animated:YES
                             completion:nil];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        [self presentViewController:composeNavigationController
                           animated:YES
                         completion:^{
                         }];
    }];
...
- (IBAction)didTapViewComments:(id)sender {
    ...
    [self performSegueWithIdentifier:@"goToCommentsSegue"
                              sender:tappedCell];
}
...
- (IBAction)didTapWriteCommentButton:(id)sender {
    ...
    [self performSegueWithIdentifier:@"goToCommentsAndWriteSegue"
                              sender:tappedCell];
}
...
- (IBAction)didTapLikeButton:(id)sender {
    ...
    [cell toggleLike:MyUser.currentUser
      withCompletion:^(BOOL succeeded, NSError * _Nullable error){
          if (error) {
              UIAlertController *alert = [ErrorAlert
                                          getErrorAlertWithTitle:@"Error toggling like"
                                          withMessage:error.localizedDescription];
